...
describe DockingStation do
	...
	describe '#release_broken_bikes' do
		it 'returns array of broken bikes' do
			dock = DockingStation.new
			2.times do dock.add_bikes(bike) end
			8.times do dock.add_bikes(double(:bike, :broken? => true, :broken= => true)) end
		expect(dock.release_broken_bikes.count).to eq 8
		end
		it 'returns array of broken bikes' do
			dock = DockingStation.new
			4.times do dock.add_bikes(bike) end
			10.times do dock.add_bikes(double(:bike, :broken? => true, :broken= => true)) end
